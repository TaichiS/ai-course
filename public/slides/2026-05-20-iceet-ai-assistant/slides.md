...
中華資訊與科技教育學會 ICEET｜2026-05-20｜詹嘉隆
...
> 目標：讓教育工作者對 Claude 建立初步信任，不只是「又一個 ChatGPT」
...
> 目標：直接回應教育工作者最大的疑慮（學生資料、研究資料）

---

## 你可能有這個疑慮

> 「把學生資料、研究數據丟給 AI，會不會外洩？」
...
**五角大廈（2026 年 2 月）：**
在法庭文件中指控 Anthropic 工具**威脅國家安全**，試圖阻止其取得政府合約。

**國安局（NSA）：**
同一時間，正在使用 Anthropic 最強大的模型 **Mythos Preview**。

> 不是矛盾——是競爭。每個部門都想搶先拿到存取權。

---

## 美英情報單位的選擇

Anthropic 將 Mythos 的使用權限「**限制在約 40 個組織**」，
理由是：攻擊性網路能力過於危險，不適合廣泛發布。

取得存取資格的包括：

- 🇺🇸 **美國國家安全局（NSA）**
- 🇬🇧 **英國情報部門**（透過英國 AI 安全研究所）
...
*[美國國家安全局採用 Anthropic 強大 AI 模型 Mythos](https://hk.news.yahoo.com/%E7%BE%8E%E5%9C%8B%E5%9C%8B%E5%AE%B6%E5%AE%89%E5%85%A8%E5%B1%80%E6%8E%A1%E7%94%A8-anthropic-%E5%BC%B7%E5%A4%A7-ai-%E6%A8%A1%E5%9E%8B-095713326.html)*
...
## Notion vs Obsidian：兩種哲學

| | Notion | Obsidian |
|--|--------|----------|
| **比喻** | 租好的雲端豪華公寓 | 私人土地上的實驗基地 |
| **資料所有權** | 存在官方雲端 | 純本地，永久持有 |
| **AI 協作** | 外部 Agent 難以進入 | 純文字，完美相容 |
| **離線使用** | 沒網路就斷手腳 | 完全離線可用 |

> **Obsidian** 適合想讓 AI 完全進入自己知識庫的人。

---

## Karpathy 的知識管理哲學

OpenAI 共同創辦人 **Andrej Karpathy** 公開分享他的 LLM 知識庫系統。
...
你的 Vault/
├── raw/        原始資料：只進不改
├── wiki/       知識庫：AI 整理好的筆記，概念互連
├── index.md    AI 自動維護的總索引
└── log.md      每次整理的變動記錄
```

**不需要向量資料庫或 embedding。**

Karpathy：「No vector databases or embeddings needed at personal scale」
直接把 Markdown 餵給 LLM 就夠了。
...
*參考資料：[當 AI 成為你的第二大腦：Karpathy 的 LLM 知識庫實踐](https://www.bnext.com.tw/article/90530/llm-knowledge-base-obsidian-claude-code)*
...
- `學術文獻摘要`：貼入論文 PDF，自動輸出重點摘要 + 研究方法存入 Obsidian
- `備課筆記整理`：雜亂的 Markdown 筆記 → 結構化教案草稿
- `學生回饋草稿`：輸入作業要點 → 生成建設性回饋範本
- `研究計畫審查`：貼入研究問題 → AI 提出邏輯漏洞與改善建議
...
**你最想把 Claude Code 用在哪個教學場景？**

歡迎提問，也歡迎分享你目前的備課或研究流程——說不定我們現場就可以幫你設計一個 Skill。
